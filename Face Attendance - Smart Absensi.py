...
def markAttendance(name):
    ...
    with open("Attendance - "+ yourclass +".csv", 'rw+') as f:
...
def absensiWajah():
    ...
    while True:
        ...
        if cv2.waitKey(1) & 0xFF == ord('q'):  # tombol q akan berhenti
            break
...
canvas.configure(bg="#1B2687")
# judul
judul = tk.Label(root, text="Sistem Absensi Dengan Face Recognition", font=("Roboto",25),bg="#4959EE", fg="white")
canvas.create_window(350, 80, window=judul)

#credit
made = tk.Label(root, text="Made by M Zaki Wiryawan & Wahyu Styo", font=("Times New Roman",13), bg="#1B2687",fg="white")
...
label1 = tk.Label(root, text="Nama Siswa", font="Roboto", fg="white", bg="#1B2687")
...
# drop= tk.OptionMenu(root, menu, *options)
# drop.pack()

canvas.create_window(457, 250, height=25, width=411, window=entry3)
label3 = tk.Label(root, text="Kelas", font="Roboto", fg="white", bg="#1B2687")
canvas.create_window(62, 250, window=label3)

global intructions

# tombol untuk rekam data wajah
intructions = tk.Label(root, text="Selamat Datang Mahasiswa UM", font=("Roboto",15),fg="white",bg="#1B2687")
canvas.create_window(370, 300, window=intructions)
Rekam_text = tk.StringVar()
Rekam_btn = tk.Button(root, textvariable=Rekam_text, font="Roboto", bg="#4959EE", fg="white", height=1, width=15,command=rekamDataWajah)
Rekam_text.set("Ambil Gambar")
Rekam_btn.grid(column=0, row=7)

# tombol untuk training wajah
Rekam_text1 = tk.StringVar()
Rekam_btn1 = tk.Button(root, textvariable=Rekam_text1, font="Roboto", bg="#4959EE", fg="white", height=1, width=15,command=trainingWajah)
Rekam_text1.set("Uji Gambar")
Rekam_btn1.grid(column=1, row=7)

# tombol absensi dengan wajah
Rekam_text2 = tk.StringVar()
Rekam_btn2 = tk.Button(root, textvariable=Rekam_text2, font="Roboto", bg="#4959EE", fg="white", height=1, width=20, command=absensiWajah)
